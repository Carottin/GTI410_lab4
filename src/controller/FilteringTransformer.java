...
public class FilteringTransformer extends AbstractTransformer{
	AnyFilter filter = new AnyFilter(new PaddingZeroStrategy(), new ImageClampStrategy(),0,0,0,0,0,0,0,0,0);
	...
	public void updateKernel(Coordinates _coordinates, float _value) {
		filter.SetValue(_coordinates.getColumn()-1,_coordinates.getRow()-1,_value);
...
//                                   + _value);
	...
	public void setClamp(String string) {
		if (string == "Abs and normalize to 255") {
			filter.setImageConversionStrategy(new AbsAndNormalizeTo255());
		}
		else if (string == "Abs and Normalize 0 to 255")
			filter.setImageConversionStrategy(new AbsAndNormalize0To255());
		else if (string == "Normalize 0 to 255")
			filter.setImageConversionStrategy(new Normalize0To255());
		else if (string == "Clamp 0...255")
