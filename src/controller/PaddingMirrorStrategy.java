	...
	public Pixel pixelAt(ImageX image, int x, int y) {
		if ((x > 0) && (x < image.getImageWidth()) && (y > 0) && (y < image.getImageHeight())) {
			...
			if (x>image.getImageWidth())
				x= image.getImageWidth()-((x-image.getImageWidth())*2);

			if (y<0)
				y = -y;
			if (y>image.getImageHeight())
				y= image.getImageHeight()-((y-image.getImageHeight())*2);
			return image.getPixel(x, y);
		}	
	}
	...
	public PixelDouble pixelAt(ImageDouble image, int x, int y) {
		if ((x > 0) && (x < image.getImageWidth()) && (y > 0) && (y < image.getImageHeight())) {
			...
			if (x<0)
				x = -x;
			if (x>=image.getImageWidth())
				x= image.getImageWidth()-((x-image.getImageWidth())*2)-1;
			if (y<0)
				y = -y;
			if (y>=image.getImageHeight())
				y= image.getImageHeight()-((y-image.getImageHeight())*2)-1;
			return image.getPixel(x, y);
		}	
	}
}
